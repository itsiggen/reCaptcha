...
cwd = os.getcwd()
file = cwd + '/subpage_list.csv'
df = pd.read_csv(file, index_col=0, header=0)
urls = df['URL'].tolist()
ids = df['AID'].tolist()

class abstractC(gym.Env):
    ...
    def reset(self):
        ...
        text = urls[idx]
        sub = ids[idx]
        self.visit(text, 1, hover, press)
        time.sleep(1)
        x, y = self.get_coords(2)
        self.goclick(x, y, 1, hover, press)
            
        self.pastsub = sub
        self.lows = 0 # number of low scores
        self.steps = 0 # step increment before score request
        # Get baseline score
    ...
    def step(self, action):
        ...
        if self.steps >= self.maxsteps:
            # Select subpage URL from the available list
            idx = randrange(0,49)
            text = urls[idx]
            self.sub = ids[idx]
            ...
            self.requests += 1
            ag.press('home')
            self.visit(text, duration, hover, press)
            time.sleep(duration)
            x, y = self.get_coords(2)
    ...
    def visit(self, text, duration, hover, press):
        x, y = self.get_coords(3)
        self.goclick(x, y, duration, hover, press)
        ag.write(text, interval=uniform(0.15, 0.2))
        ag.press('enter')
                
    def typing(self, duration, hover, press):
        x, y = self.get_coords(4)
        self.goclick(x, y, duration, hover, press)
        num = choice([5,7,9])
        for i in range(num):
    ...
    def gen_reward(self):
        ...
        if len(self.tabscores[self.tab]) <= 2:
            avg = self.base
        else:
    ...
    def log_results(self):
        ...
        file_path = os.path.join(file_dir, csv_folder, 'abstractC-'+self.hour+'.csv')
